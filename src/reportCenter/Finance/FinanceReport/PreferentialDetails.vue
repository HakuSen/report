...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="PreferentialDetailsLoadingFlag" :loadingName="'PreferentialDetailsLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData" :pageSizes="pageSizes"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getPreferentialDetailsList',
        dataName: 'tablePreferentialDetails'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tablePreferentialDetails,
      tableParams: State => State.info.params.PreferentialDetails.tablePreferentialDetailsNewParams,
      ...
      everyTableParams: State => State.info.params.PreferentialDetails.tablePreferentialDetailsParams,
      PreferentialDetailsLoadingFlag: State => State.info.PreferentialDetailsLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.PreferentialDetailsLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'PreferentialDetailsLoading', loading: true });
      // params 公用部分提出
       ...
       */
      let newParams = {
        url: 'getPreferentialDetailsList',
        tableData: 'tablePreferentialDetails',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      params['page_size'] = 100;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tablePreferentialDetailsNewParams', newParamsName: 'PreferentialDetails' });
      if (this.formData) {
       ...
       * 如果时间有空，不查询接口
       * */
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['PreferentialDetailsLoading', 'tablePreferentialDetailsParams', 'PreferentialDetails']);
    },
    /**
     * 重置表单数据
     */
  ...
  mounted() {
    sendOnceQuery(this.$store, 'PreferentialDetails', 'tablePreferentialDetailsParams', this.tableData, this.urlParams, 'PreferentialDetailsLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.PreferentialDetailsLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tablePreferentialDetails' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'PreferentialDetailsLoading', loading: false });
    next(true);
  }
};
</script>

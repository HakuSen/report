...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      ...
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingName="'shopTargetAnalysisLoading'" :loadingFlag="shopTargetAnalysisLoadingFlag" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getShopTargetAnalysisList',
        dataName: 'tableShopTargetAnalysis'
      } // 接口请求
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableShopTargetAnalysis,
      tableParams: State => State.info.params.shopTargetAnalysis.tableShopTargetAnalysisNewParams,
      ...
      everyTableParams: State => State.info.params.shopTargetAnalysis.tableShopTargetAnalysisParams,
      queryLoading: State => State.info.queryLoading,
      shopTargetAnalysisLoadingFlag: State => State.info.shopTargetAnalysisLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                indicators: true,
                options: [
                  { label: '营业额', value: 'originalPrice' },
                  { label: '实收金额', value: 'totalPrice' },
                  { label: '客流量', value: 'consumerCount' },
                  { label: '订单量', value: 'orders' },
                  { label: '日均营业额', value: 'dayOriginalPirce' },
                  { label: '日均客流量', value: 'dayConsumerCount' },
                  { label: '日均订单量', value: 'dayOrders' },
                  { label: '人均消费', value: 'consumerOriginalPrice' }
                ],
                attrs: {
                  size: ComponentSize
                }
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.shopTargetAnalysisLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopTargetAnalysisLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      if(this.formData.indicators) {
        params['compare_key_name'] = this.formData.indicators.split(';')[0];
        params['compare_china_key_name'] = this.formData.indicators.split(';')[1];
      }
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['shopTargetAnalysisLoading', 'tableShopTargetAnalysisParams', 'shopTargetAnalysis']);
      let newParams = {
        url: 'getShopTargetAnalysisList',
        tableData: 'tableShopTargetAnalysis',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableShopTargetAnalysisNewParams', newParamsName: 'shopTargetAnalysis' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'shopTargetAnalysis', 'tableShopTargetAnalysisParams', this.tableData, this.urlParams, 'shopTargetAnalysisLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.shopTargetAnalysisLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableShopTargetAnalysis' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopTargetAnalysisLoading', loading: false });
    next(true);
  }
};
</script>

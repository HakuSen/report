...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingName="'GoodsShopAnalysisLoading'" :loadingFlag="GoodsShopAnalysisLoadingFlag" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getGoodsShopAnalysisList',
        dataName: 'tableGoodsShopAnalysis'
      } // 接口请求
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableGoodsShopAnalysis,
      tableParams: State => State.info.params.goodsShopAnalysis.tableGoodsShopAnalysiNewParams,
      ...
      everyTableParams: State => State.info.params.goodsShopAnalysis.tableGoodsShopAnalysisParams,
      GoodsShopAnalysisLoadingFlag: State => State.info.GoodsShopAnalysisLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.GoodsShopAnalysisLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'GoodsShopAnalysisLoading', loading: true });
      ...
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['GoodsShopAnalysisLoading', 'tableGoodsShopAnalysisParams',
        'goodsShopAnalysis']);
      let newParams = {
        url: 'getGoodsShopAnalysisList',
        tableData: 'tableGoodsShopAnalysis',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableGoodsShopAnalysiNewParams', newParamsName: 'goodsShopAnalysis' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'goodsShopAnalysis', 'tableGoodsShopAnalysisParams', this.tableData, this.urlParams, 'GoodsShopAnalysisLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.GoodsShopAnalysisLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableGoodsShopAnalysis' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'GoodsShopAnalysisLoading', loading: false });
    next(true);
  }
};
</script>

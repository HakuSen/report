...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="goodsStoreAnalyzeLoadingFlag" :loadingName="'goodsStoreAnalyzeLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
import YhProperty from '../../../components/base/yinhe/YhProperty';
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getGoodsStoreAnalyzeList',
        dataName: 'tableGoodsStoreAnalyze'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableGoodsStoreAnalyze,
      tableParams: State => State.info.params.goodsStoreAnalyze.tableGoodsStoreAnalyzeNewParams,
      ...
      everyTableParams: State => State.info.params.goodsStoreAnalyze.tableGoodsStoreAnalyzeParams,
      goodsStoreAnalyzeLoadingFlag: State => State.info.goodsStoreAnalyzeLoading
    }),
    syncConfig() {
      return {
        key: 'two',
        ...
        config: [
          ...
          },
          {
            ...
            children: [
              {
                name: 'yh-property',
                getSingle: true, // 商品名称单选时为true，多选时为false
                typeShow: false, // 业务类型
                label: ['属性分类：', '业务类型：', '属性名称：'],
                fieldName: 'property',
                value: [],
                url: ['/property/findClassSelect', '/property/findPropType', '/property/findProp'],
                attrs: {
                  size: ComponentSize
                }
              }
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.goodsStoreAnalyzeLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsStoreAnalyzeLoading', loading: true });
      ...
      if (this.formData.property[0] !== '') {
        params['prop_class_id'] = this.formData.property[0];
      }
      if (this.formData.property[1] !== '') {
        params['prop_type_id'] = this.formData.property[1];
      }
      params['prop_id'] = this.formData.property[2];
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['goodsStoreAnalyzeLoading', 'tableGoodsStoreAnalyzeParams', 'goodsStoreAnalyze']);
      let newParams = {
        url: 'getGoodsStoreAnalyzeList',
        tableData: 'tableGoodsStoreAnalyze',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableGoodsStoreAnalyzeNewParams', newParamsName: 'goodsStoreAnalyze' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'goodsStoreAnalyze', 'tableGoodsStoreAnalyzeParams', this.tableData, this.urlParams, 'goodsStoreAnalyzeLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.goodsStoreAnalyzeLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableGoodsStoreAnalyze' });
        }
      }
      vm.titleArrayNum = 0;
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsStoreAnalyzeLoading', loading: false });
    next(true);
  }
};
</script>

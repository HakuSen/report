...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="goodsDataLoadingFlag" :loadingName="'goodsDataLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      urlParams: {
        ...
        dataName: 'tableSalesDataAnalysis'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableSalesDataAnalysis,
      tableParams: State => State.info.params.goodsAnalyzeData.tableGoodsDataAnalyzeNewParams,
      ...
      everyTableParams: State => State.info.params.goodsAnalyzeData.tableGoodsDataAnalyzeParams,
      goodsDataLoadingFlag: State => State.info.goodsDataLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.goodsDataLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
     ...
     */
    submitHandler() {
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsDataLoading', loading: true });
      ...
      let newParams = {
        ...
        tableData: 'tableSalesDataAnalysis',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableGoodsDataAnalyzeNewParams', newParamsName: 'goodsAnalyzeData' });
      if (this.formData) {
        this.$store.dispatch('info/formData', this.formData);
      }
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['goodsDataLoading', 'tableGoodsDataAnalyzeParams',
        'goodsAnalyzeData'], this.formData);
    },
    /**
     * 重置表单数据
     */
  ...
  mounted() {
    sendOnceQuery(this.$store, 'goodsAnalyzeData', 'tableGoodsDataAnalyzeParams', this.tableData, this.urlParams, 'goodsDataLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.goodsDataLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableSalesDataAnalysis' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsDataLoading', loading: false });
    next(true);
  }
};
</script>

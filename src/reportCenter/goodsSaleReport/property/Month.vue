...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="goodsMonthLoadingFlag" :loadingName="'goodsMonthLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      needParams: {
        ...
        contentText: '商品属性销售分析',
        contentShow: false,
        shopShowFlag: true,
        mergeTextArray: ['propName', 'propName'],
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getGoodsAnalyzeList',
        dataName: 'tableSalesMonthAnalysis'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableSalesMonthAnalysis,
      tableParams: State => State.info.params.goodsAnalyzeMonth.tableGoodsMonthAnalyzeNewParams,
      ...
      everyTableParams: State => State.info.params.goodsAnalyzeMonth.tableGoodsMonthAnalyzeParams,
      goodsMonthLoadingFlag: State => State.info.goodsMonthLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.goodsMonthLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
     ...
     */
    submitHandler() {
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsMonthLoading', loading: true });
      ...
      if (this.formData.property[2] && this.formData.property[2][0] !== '全部') {
        params['prop_ids'] = this.formData.property[2];
      }
      // params.type = 3;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['goodsMonthLoading', 'tableGoodsMonthAnalyzeParams',
        'goodsAnalyzeMonth'], this.formData);
      let newParams = {
        url: 'getGoodsAnalyzeList',
        tableData: 'tableSalesMonthAnalysis',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableGoodsMonthAnalyzeNewParams', newParamsName: 'goodsAnalyzeMonth' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'goodsAnalyzeMonth', 'tableGoodsMonthAnalyzeParams', this.tableData, this.urlParams, 'goodsMonthLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.goodsMonthLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableSalesMonthAnalysis' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsMonthLoading', loading: false });
    next(true);
  }
};
</script>

...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="goodsWeekLoadingFlag" :loadingName="'goodsWeekLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      urlParams: {
        ...
        dataName: 'tableSalesWeekAnalysis'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableSalesWeekAnalysis,
      tableParams: State => State.info.params.goodsAnalyzeWeek.tableGoodsWeekAnalyzeNewParams,
      ...
      everyTableParams: State => State.info.params.goodsAnalyzeWeek.tableGoodsWeekAnalyzeParams,
      goodsWeekLoadingFlag: State => State.info.goodsWeekLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.goodsWeekLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
     ...
     */
    submitHandler() {
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsWeekLoading', loading: true });
      ...
      params.type = 2;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['goodsWeekLoading', 'tableGoodsWeekAnalyzeParams',
        'goodsAnalyzeWeek'], this.formData);
      let newParams = {
        url: 'getGoodsAnalyzeList',
        tableData: 'tableSalesWeekAnalysis',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableGoodsWeekAnalyzeNewParams', newParamsName: 'goodsAnalyzeWeek' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'goodsAnalyzeWeek', 'tableGoodsWeekAnalyzeParams', this.tableData, this.urlParams, 'goodsWeekLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.goodsWeekLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableSalesWeekAnalysis' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'goodsWeekLoading', loading: false });
    next(true);
  }
};
</script>

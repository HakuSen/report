...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="shopTimeIncomeLoadingFlag" :loadingName="'shopTimeIncomeLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getShopTimeIncomeList',
        dataName: 'shopTimeData'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.shopTimeData,
      tableParams: State => State.info.params.shopTimeIncome.shopTimeDataNewParams,
      ...
      everyTableParams: State => State.info.params.shopTimeIncome.shopTimeDataParams,
      shopTimeIncomeLoadingFlag: State => State.info.shopTimeIncomeLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.shopTimeIncomeLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopTimeIncomeLoading', loading: true });
       ...
       */
      let newParams = {
        url: 'getShopTimeIncomeList',
        tableData: 'shopTimeData',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'shopTimeDataNewParams', newParamsName: 'shopTimeIncome' });
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['shopTimeIncomeLoading', 'shopTimeDataParams', 'shopTimeIncome']);
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'shopTimeIncome', 'shopTimeDataParams', this.tableData, this.urlParams, 'shopTimeIncomeLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.shopTimeIncomeLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'shopTimeData' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopTimeIncomeLoading', loading: false });
    next(true);
  }
};
</script>

...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingName="'revenueTotalLoading'" :loadingFlag="revenueTotalLoadingFlag" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getRevenueList',
        dataName: 'tableRevenueData'
      } // 接口请求
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableRevenueData,
      tableParams: State => State.info.params.revenueIncome.tableRevenueDataNewParams,
      ...
      everyTableParams: State => State.info.params.revenueIncome.tableRevenueDataParams,
      queryLoading: State => State.info.queryLoading,
      revenueTotalLoadingFlag: State => State.info.revenueTotalLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.revenueTotalLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'revenueTotalLoading', loading: true });
      ...
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['revenueTotalLoading', 'tableRevenueDataParams', 'revenueIncome']);
      let newParams = {
        url: 'getRevenueList',
        tableData: 'tableRevenueData',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableRevenueDataNewParams', newParamsName: 'revenueIncome' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'revenueIncome', 'tableRevenueDataParams', this.tableData, this.urlParams, 'revenueTotalLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.revenueTotalLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableRevenueData' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'revenueTotalLoading', loading: false });
    next(true);
  }
};
</script>

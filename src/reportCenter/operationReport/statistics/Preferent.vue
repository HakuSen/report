...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="preferentCountLoadingFlag" :loadingName=" 'preferentCountLoading' " :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getPreferentCountList',
        dataName: 'tablePreferentCount'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tablePreferentCount,
      tableParams: State => State.info.params.preferentCount.tablePreferentCountNewParams,
      ...
      everyTableParams: State => State.info.params.preferentCount.tablePreferentCountParams,
      preferentCountLoadingFlag: State => State.info.preferentCountLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.preferentCountLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'preferentCountLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['preferentCountLoading', 'tablePreferentCountParams', 'preferentCount']);
      let newParams = {
        url: 'getPreferentCountList',
        tableData: 'tablePreferentCount',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tablePreferentCountNewParams', newParamsName: 'preferentCount' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'preferentCount', 'tablePreferentCountParams', this.tableData, this.urlParams, 'preferentCountLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.preferentCountLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tablePreferentCount' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'preferentCountLoading', loading: false });
    next(true);
  }
};
</script>

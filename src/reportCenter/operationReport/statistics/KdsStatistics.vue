...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      ...
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="kdsStatisticsLoadingFlag" :tableFlag="tableFlag" :loadingName="'kdsStatisticsLoading'" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
import YhSingleShopEquip from '../../../components/base/yinhe/YhSingleShopEquip';
...
export default {
  ...
  data() {
    return {
      ...
      message: '每日14点及21点上传数据',
      pageNum: 0,
      tableFlag: false,
      newPageingParams: {},
      totalData: [],
      urlParams: {
        interfaceAddress: 'getKdsStatisticsList',
        dataName: 'tableKdsStatistics'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableKdsStatistics,
      tableParams: State => State.info.params.kdsStatistics.tableKdsStatisticsNewParams,
      ...
      everyTableParams: State => State.info.params.kdsStatistics.tableKdsStatisticsParams,
      kdsStatisticsLoadingFlag: State => State.info.kdsStatisticsLoading
    }),
    syncConfig() {
      return {
        key: 'two',
        ...
        config: [
          ...
          },
          {
            ...
            children: [
              {
                name: 'yh-single-shop-equip',
                label: ['店铺选择：', '设备选择：'],
                fieldName: 'shopEquip',
                value: [],
                url: ['/shopinfo/findStructureStores', '/machine/findMachineHardwareList'],
                attrs: {
                  size: ComponentSize
                }
              }
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.kdsStatisticsLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    closePayList() {
      dropdownBoxBack(this.totalData, this.$refs, ['yh-pay', 'yh-property', 'yh-single-shop-equip']);
    },
    /**
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'kdsStatisticsLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      params['shopper_id'] = this.formData['shopEquip'][0];
      params['hardware_sns'] = this.formData['shopEquip'][1];
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['kdsStatisticsLoading', 'tableKdsStatisticsParams', 'kdsStatistics']);
      let newParams = {
        url: 'getKdsStatisticsList',
        tableData: 'tableKdsStatistics',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableKdsStatisticsNewParams', newParamsName: 'kdsStatistics' });
      if (this.formData) {
  ...
  updated() {
    sendOnceQuery(this.$store, 'kdsStatistics', 'tableKdsStatisticsParams', this.tableData, this.urlParams, 'kdsStatisticsLoading');
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.kdsStatisticsLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableKdsStatistics' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'kdsStatisticsLoading', loading: false });
    next(true);
  }
};
</script>

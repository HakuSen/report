...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" :loadingName="'storeTimeRefundLoading'" @tableShow="tableShow" :loadingFlag="storeTimeRefundLoadingFlag" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getStoreTimeRefundList',
        dataName: 'tableStoreTimeRefund'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableStoreTimeRefund,
      tableParams: State => State.info.params.storeTimeRefund.tableStoreTimeRefundNewParams,
      ...
      everyTableParams: State => State.info.params.storeTimeRefund.tableStoreTimeRefundParams,
      storeTimeRefundLoadingFlag: State => State.info.storeTimeRefundLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.storeTimeRefundLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'storeTimeRefundLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['storeTimeRefundLoading', 'tableStoreTimeRefundParams', 'storeTimeRefund']);
      let newParams = {
        url: 'getStoreTimeRefundList',
        tableData: 'tableStoreTimeRefund',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableStoreTimeRefundNewParams', newParamsName: 'storeTimeRefund' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'storeTimeRefund', 'tableStoreTimeRefundParams', this.tableData, this.urlParams, 'storeTimeRefundLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.storeTimeRefundLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableStoreTimeRefund' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'storeTimeRefundLoading', loading: false });
    next(true);
  }
};
</script>

...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="refundCountLoadingFlag" :tableFlag="tableFlag" :loadingName="'refundCountLoading'" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getRefundCountList',
        dataName: 'tableRefundCount'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableRefundCount,
      tableParams: State => State.info.params.refundCount.tableRefundCountNewParams,
      ...
      everyTableParams: State => State.info.params.refundCount.tableRefundCountParams,
      refundCountLoadingFlag: State => State.info.refundCountLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.refundCountLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'refundCountLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['refundCountLoading', 'tableRefundCountParams', 'refundCount']);
      let newParams = {
        url: 'getRefundCountList',
        tableData: 'tableRefundCount',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableRefundCountNewParams', newParamsName: 'refundCount' });
      if (this.formData) {
  ...
  updated() {
    sendOnceQuery(this.$store, 'refundCount', 'tableRefundCountParams', this.tableData, this.urlParams, 'refundCountLoading');
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.refundCountLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableRefundCount' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'refundCountLoading', loading: false });
    next(true);
  }
};
</script>

<!-------------------------
 ...
 ----------------------- -->
<template>
  <div class="account" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      ...
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="accountLoadingFlag" :loadingName="'accountLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      newPageingParams: {},
      totalData: [],
      message: '因挂账业务升级，客户为空的为升级前的旧数据，请到门店机器上进行对账',
      urlParams: {
        interfaceAddress: 'getAccountList',
        dataName: 'tableAccountData'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableAccountData,
      tableParams: State => State.info.params.accountTotal.tableAccountTotalNewParams,
      ...
      everyTableParams: State => State.info.params.accountTotal.tableAccountTotalParams,
      accountLoadingFlag: State => State.info.accountLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                isClient: true,
                value: [],
                url: '/onAccount/findOnAccountTartgerList',
                attrs: {
                  size: ComponentSize
                }
              }
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.accountLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    closePayList() {
      dropdownBoxBack(this.totalData, this.$refs, ['yh-pay']);
    },
    /**
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'accountLoading', loading: true });
      // params 公用部分提出
       ...
       */
      if (this.formData.payType[0] === '全部' || this.formData.payType.length === 0) {
        params['customer_uuidlist'] = null;
      }
      else {
        params['customer_uuidlist'] = this.formData.payType;
      }
      params['shopper_ids'] = this.userinfo['shopper_id'];
      let newParams = {
        url: 'getAccountList',
      ...
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableAccountTotalNewParams', newParamsName: 'accountTotal' });
      if (this.formData) {
       ...
       * 如果时间有空，不查询接口
       * */
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['accountLoading', 'tableAccountTotalParams', 'accountTotal']);
    },
    /**
     * 重置表单数据
     */
  ...
  mounted() {
    sendOnceQuery(this.$store, 'accountTotal', 'tableAccountTotalParams', this.tableData, this.urlParams, 'accountLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.accountLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableAccountData' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'accountLoading', loading: false });
    next(true);
  }
};
</script>

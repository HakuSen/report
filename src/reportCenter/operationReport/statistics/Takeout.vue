...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="takeoutStatisticsLoadingFlag" :loadingName="'takeoutStatisticsLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getTakeoutStatisticsList',
        dataName: 'tableTakeoutStatistics'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableTakeoutStatistics,
      tableParams: State => State.info.params.takeoutStatistics.takeoutStatisticsNewParams,
      ...
      everyTableParams: State => State.info.params.takeoutStatistics.takeoutStatisticsParams,
      takeoutStatisticsLoadingFlag: State => State.info.takeoutStatisticsLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.takeoutStatisticsLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'takeoutStatisticsLoading', loading: true });
      // params 公用部分提出
       ...
       */
      let newParams = {
        url: 'getTakeoutStatisticsList',
        tableData: 'tableTakeoutStatistics',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'takeoutStatisticsNewParams', newParamsName: 'takeoutStatistics' });
      if (this.formData) {
       ...
       * 如果时间有空，不查询接口
       * */
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['takeoutStatisticsLoading', 'takeoutStatisticsParams', 'takeoutStatistics']);
    },
    /**
     * 重置表单数据
     */
  ...
  mounted() {
    sendOnceQuery(this.$store, 'takeoutStatistics', 'takeoutStatisticsParams', this.tableData, this.urlParams, 'takeoutStatisticsLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.takeoutStatisticsLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableTakeoutStatistics' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'takeoutStatisticsLoading', loading: false });
    next(true);
  }
};
</script>

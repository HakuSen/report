  ...
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams"
                  :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingName="'storeRefundCountLoading'"
                  :loadingFlag="storeRefundCountLoadingFlag" :tableFlag="tableFlag"
                  :newPageing="newPageingParams" :formData="formData"></base-table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getStoreRefundCountList',
        dataName: 'tableStoreRefundCount'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableStoreRefundCount,
      tableParams: State => State.info.params.storeRefundCount.tableStoreRefundCountNewParams,
      ...
      everyTableParams: State => State.info.params.storeRefundCount.tableStoreRefundCountParams,
      storeRefundCountLoadingFlag: State => State.info.storeRefundCountLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.storeRefundCountLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'storeRefundCountLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['storeRefundCountLoading', 'tableStoreRefundCountParams', 'storeRefundCount']);
      let newParams = {
        url: 'getStoreRefundCountList',
        tableData: 'tableStoreRefundCount',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', {
        newParams,
        oneTableName: 'tableStoreRefundCountNewParams',
        newParamsName: 'storeRefundCount'
      });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'storeRefundCount', 'tableStoreRefundCountParams', this.tableData, this.urlParams, 'storeRefundCountLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.storeRefundCountLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableStoreRefundCount' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'storeRefundCountLoading', loading: false });
    next(true);
  }
};
</script>

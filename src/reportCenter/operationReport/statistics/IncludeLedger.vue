...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="includeLedgerLoadingFlag" :loadingName="'includeLedgerLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getIncludeLedgerList',
        dataName: 'tableIncludeLedger'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableIncludeLedger,
      tableParams: State => State.info.params.includeLedger.includeLedgerNewParams,
      ...
      everyTableParams: State => State.info.params.includeLedger.includeLedgerParams,
      includeLedgerLoadingFlag: State => State.info.includeLedgerLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.includeLedgerLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'includeLedgerLoading', loading: true });
      // params 公用部分提出
       ...
       */
      let newParams = {
        url: 'getIncludeLedgerList',
        tableData: 'tableIncludeLedger',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'includeLedgerNewParams', newParamsName: 'includeLedger' });
      if (this.formData) {
       ...
       * 如果时间有空，不查询接口
       * */
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['includeLedgerLoading', 'includeLedgerParams', 'includeLedger']);
    },
    /**
     * 重置表单数据
     */
  ...
  mounted() {
    sendOnceQuery(this.$store, 'includeLedger', 'includeLedgerParams', this.tableData, this.urlParams, 'includeLedgerLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.includeLedgerLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableIncludeLedger' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'includeLedgerLoading', loading: false });
    next(true);
  }
};
</script>

...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="shopIncomeLoadingFlag" :loadingName="'shopIncomeLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getStoreIncomeList',
        dataName: 'tableStoreIncome'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableStoreIncome,
      tableParams: State => State.info.params.shopIncome.tableStoreIncomeNewParams,
      ...
      everyTableParams: State => State.info.params.shopIncome.tableStoreIncomeParams,
      shopIncomeLoadingFlag: State => State.info.shopIncomeLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.shopIncomeLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
     ...
     */
    submitHandler() {
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopIncomeLoading', loading: true });
       ...
       */
      let newParams = {
        url: 'getStoreIncomeList',
        tableData: 'tableStoreIncome',
        pageingFlag: ++this.pageNum
      };
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableStoreIncomeNewParams', newParamsName: 'shopIncome' });
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['shopIncomeLoading', 'tableStoreIncomeParams', 'shopIncome']);
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'shopIncome', 'tableStoreIncomeParams', this.tableData, this.urlParams, 'shopIncomeLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.shopIncomeLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableStoreIncome' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopIncomeLoading', loading: false });
    next(true);
  }
};
</script>

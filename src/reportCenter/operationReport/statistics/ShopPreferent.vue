...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="storePreferentLoadingFlag" :tableFlag="tableFlag" :loadingName=" 'storePreferentLoading' " :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getStorePreferentList',
        dataName: 'tableStorePreferent'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableStorePreferent,
      tableParams: State => State.info.params.storePreferent.tableStorePreferentNewParams,
      ...
      everyTableParams: State => State.info.params.storePreferent.tableStorePreferentParams,
      storePreferentLoadingFlag: State => State.info.storePreferentLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.storePreferentLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'storePreferentLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['storePreferentLoading', 'tableStorePreferentParams', 'storePreferent']);
      let newParams = {
        url: 'getStorePreferentList',
        tableData: 'tableStorePreferent',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableStorePreferentNewParams', newParamsName: 'storePreferent' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'storePreferent', 'tableStorePreferentParams', this.tableData, this.urlParams, 'this.storePreferentLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.storePreferentLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableStorePreferent' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'storePreferentLoading', loading: false });
    next(true);
  }
};
</script>

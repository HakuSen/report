...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="incomeLoadingFlag" :loadingName="'incomeLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getIncomeLedgerList',
        dataName: 'tableIncomeTotalData'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableIncomeTotalData,
      tableParams: State => State.info.params.incomeTotal.tableIncomeTotalNewParams,
      ...
      everyTableParams: State => State.info.params.incomeTotal.tableIncomeTotalParams,
      incomeLoadingFlag: State => State.info.incomeLoading
    }),
    syncConfig() {
      return {
        key: 'two',
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'incomeLoading', loading: true });
      // params 公用部分提出
       ...
       */
      let newParams = {
        url: 'getIncomeLedgerList',
        tableData: 'tableIncomeTotalData',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableIncomeTotalNewParams', newParamsName: 'incomeTotal' });
      if (this.formData) {
       ...
       * 如果时间有空，不查询接口
       * */
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['incomeLoading', 'tableIncomeTotalParams', 'incomeTotal']);
    },
    /**
     * 重置表单数据
     */
  ...
  mounted() {
    sendOnceQuery(this.$store, 'incomeTotal', 'tableIncomeTotalParams', this.tableData, this.urlParams, 'incomeLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.incomeLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableIncomeTotalData' });
        }
      }
      vm.titleArrayNum = 0;
      // 首次添加页面
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'incomeLoading', loading: false });
    next(true);
  }
};
</script>

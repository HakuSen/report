...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingName="'shopRevenueLoading'" :loadingFlag="shopRevenueLoadingFlag" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getRevenueShopList',
        dataName: 'tableRevenueShop'
      } // 接口请求
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableRevenueShop,
      tableParams: State => State.info.params.revenueShopIncome.tableRevenueShopNewParams,
      ...
      everyTableParams: State => State.info.params.revenueShopIncome.tableRevenueShopParams,
      shopRevenueLoadingFlag: State => State.info.shopRevenueLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.shopRevenueLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopRevenueLoading', loading: true });
      ...
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['shopRevenueLoading', 'tableRevenueShopParams', 'revenueShopIncome']);
      let newParams = {
        url: 'getRevenueShopList',
        tableData: 'tableRevenueShop',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableRevenueShopNewParams', newParamsName: 'revenueShopIncome' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'revenueShopIncome', 'tableRevenueShopParams', this.tableData, this.urlParams, 'this.shopRevenueLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.shopRevenueLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableRevenueShop' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopRevenueLoading', loading: false });
    next(true);
  }
};
</script>

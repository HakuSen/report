...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="storeTimePreferentLoadingFlag" :tableFlag="tableFlag" :loadingName=" 'storeTimePreferentLoading' " :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getStoreTimePreferentList',
        dataName: 'tableStoreTimePreferent'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableStoreTimePreferent,
      tableParams: State => State.info.params.storeTimePreferent.tableStoreTimePreferentNewParams,
      ...
      everyTableParams: State => State.info.params.storeTimePreferent.tableStoreTimePreferentParams,
      storeTimePreferentLoadingFlag: State => State.info.storeTimePreferentLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.storeTimePreferentLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'storeTimePreferentLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['storeTimePreferentLoading', 'tableStoreTimePreferentParams',
        'storeTimePreferent']);
      let newParams = {
        url: 'getStoreTimePreferentList',
        tableData: 'tableStoreTimePreferent',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableStoreTimePreferentNewParams', newParamsName: 'storeTimePreferent' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'storeTimePreferent', 'tableStoreTimePreferentParams', this.tableData, this.urlParams, 'storeTimePreferentLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.storeTimePreferentLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableStoreTimePreferent' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'storeTimePreferentLoading', loading: false });
    next(true);
  }
};
</script>

...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      ...
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="shopReceiptsLoadingFlag" :tableFlag="tableFlag" :loadingName="'shopReceiptsLoading'" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      urlParams: {
        interfaceAddress: 'getShopReceiptsList',
        dataName: 'shopReceipts'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.shopReceipts,
      tableParams: State => State.info.params.shopReceipts.shopReceiptsNewParams,
      ...
      everyTableParams: State => State.info.params.shopReceipts.shopReceiptsParams,
      shopReceiptsLoadingFlag: State => State.info.shopReceiptsLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.shopReceiptsLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopReceiptsLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      if (this.formData['checkGroup'].length > 0) {
        params['outer_support_id'] = '1';
      }
      if (this.formData['indent'] === '0') {
        params['order_type_ts'] = 'tangshi';
      }
      else if (this.formData['indent'] === '1') {
        params['order_type_wm'] = 'waimai';
      }
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['shopReceiptsLoading', 'shopReceiptsParams', 'shopReceipts']);
      let newParams = {
        url: 'getShopReceiptsList',
        tableData: 'shopReceipts',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'shopReceiptsNewParams', newParamsName: 'shopReceipts' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'shopReceipts', 'shopReceiptsParams', this.tableData, this.urlParams, 'shopReceiptsLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.shopReceiptsLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'shopReceipts' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopReceiptsLoading', loading: false });
    next(true);
  }
};
</script>

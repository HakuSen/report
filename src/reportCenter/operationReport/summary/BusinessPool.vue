...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="businPoolLoadingFlag" :loadingName="'businPoolLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getBusinessPoolList',
        dataName: 'tableBusinessPool'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableBusinessPool,
      tableParams: State => State.info.params.businessPool.tableBusinessPoolNewParams,
      ...
      everyTableParams: State => State.info.params.businessPool.tableBusinessPoolParams,
      businPoolLoadingFlag: State => State.info.businPoolLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                label: ['店铺选择：', '机器选择：', '员工选择：'],
                fieldName: 'shopMachine',
                value: [],
                isEmployees: true,
                isMachine: true,
                url: ['/shopinfo/findStructureStores', '/machine/findMachineList', '/employee/findEmployeesByShopperIdList'],
                attrs: {
                  size: ComponentSize
                }
              }
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.businPoolLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'businPoolLoading', loading: true });
      ...
      if (this.formData['shopMachine'].length > 0 && this.formData['shopMachine'][2] !== '' && this.formData['shopMachine'][2] !== 'all') {
        params['job_num'] = this.formData['shopMachine'][2];
      }
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['businPoolLoading', 'tableBusinessPoolParams', 'businessPool']);
      let newParams = {
        url: 'getBusinessPoolList',
        tableData: 'tableBusinessPool',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'tableBusinessPoolNewParams', newParamsName: 'businessPool' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'businessPool', 'tableBusinessPoolParams', this.tableData, this.urlParams, 'businPoolLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.businPoolLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableBusinessPool' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'businPoolLoading', loading: false });
    next(true);
  }
};
</script>

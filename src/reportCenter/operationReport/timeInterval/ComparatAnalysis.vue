...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <base-table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="timeComparatAnalysisLoadingFlag" :loadingName="'timeComparatAnalysisLoading'" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"/>
    </div>
  </div>
</template>
...
import YhEqualityInterval from '../../../components/base/yinhe/YhEqualityInterval';
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'timeComparatAnalysisList',
        dataName: 'tableTimeComparatAnalysis'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableTimeComparatAnalysis,
      tableParams: State => State.info.params.timeComparatAnalysis.timeComparatAnalysisNewParams,
      ...
      everyTableParams: State => State.info.params.timeComparatAnalysis.timeComparatAnalysisParams,
      timeComparatAnalysisLoadingFlag: State => State.info.timeComparatAnalysisLoading
    }),
    syncConfig() {
      return {
        key: 'two',
        ...
        config: [
          {
            ...
            children: [
              {
                name: 'yh-equality-interval',
                label: ['本期时间：', '上期时间：'],
                fieldName: 'equalityInterval',
                value: [],
                lastDefault: true,
                options: [],
                attrs: {
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.timeComparatAnalysisLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'timeComparatAnalysisLoading', loading: true });
      // params 公用部分提出
       ...
       */
      let newParams = {
        url: 'timeComparatAnalysisList',
        tableData: 'tableTimeComparatAnalysis',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'timeComparatAnalysisNewParams', newParamsName: 'timeComparatAnalysis' });
      if (this.formData) {
       ...
       * 如果时间有空，不查询接口
       * */
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['timeComparatAnalysisLoading', 'timeComparatAnalysisParams',
  ...
  mounted() {
    sendOnceQuery(this.$store, 'timeComparatAnalysis', 'timeComparatAnalysisParams', this.tableData, this.urlParams, 'timeComparatAnalysisLoading');
  ...
  updated() {
    ...
    this.totalData[l - 2].children[0].attrs['loading'] = this.timeComparatAnalysisLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableTimeComparatAnalysis' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'timeComparatAnalysisLoading', loading: false });
    next(true);
  }
};
</script>

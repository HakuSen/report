...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingName="'solidVoucherLoading'" :loadingFlag="solidVoucherLoadingFlag" :tableFlag="tableFlag" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'findShopVoucher',
        dataName: 'solidVoucher'
      } // 接口请求
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.solidVoucher,
      tableParams: State => State.info.params.SolidVoucherReport.solidVoucherNewParams,
      ...
      everyTableParams: State => State.info.params.SolidVoucherReport.solidVoucherParams,
      solidVoucherLoadingFlag: State => State.info.solidVoucherLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.solidVoucherLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'solidVoucherLoading', loading: true });
      ...
      //   params['pay_model_list'] = [];
      // }
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['solidVoucherLoading', 'solidVoucherParams', 'SolidVoucherReport']);
      let newParams = {
        url: 'findShopVoucher',
        tableData: 'solidVoucher',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'solidVoucherNewParams', newParamsName: 'SolidVoucherReport' });
      if (this.formData) {
  ...
  mounted() {
    sendOnceQuery(this.$store, 'SolidVoucherReport', 'solidVoucherParams', this.tableData, this.urlParams, 'this.solidVoucherLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.solidVoucherLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'solidVoucher' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'solidVoucherLoading', loading: false });
    next(true);
  }
};
</script>

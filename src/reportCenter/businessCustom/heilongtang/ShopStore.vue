...
<template>
  <div class="income" @click="closePayList">
    ...
    <div class="table-warp" style="background: #fff;">
      <Table :tableAllData="tableData" :queryParams="tableParams" :tableShowParams="needParams" :tableQueryParams="everyTableParams" @tableShow="tableShow" :loadingFlag="shopStoreLoadingFlag" :tableFlag="tableFlag" :loadingName="'shopStoreLoading'" :surchargeName="surchargeUrl" :newPageing="newPageingParams" :formData="formData"></Table>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      surchargeUrl: 'electronicDetailList',
      formData: {},
      needParams: {
        contactArray: [],
        contactDot: 0,
      ...
      totalData: [],
      urlParams: {
        interfaceAddress: 'getShopStoreList',
        dataName: 'tableShopStore'
      } // 接口请求 方法映射
    };
  },
  computed: {
    ...
    ...mapState({
      tableData: State => State.info.tableShopStore,
      tableParams: State => State.info.params.shopStore.shopStoreNewParams,
      ...
      everyTableParams: State => State.info.params.shopStore.shopStoreParams,
      shopStoreLoadingFlag: State => State.info.shopStoreLoading
    }),
    syncConfig() {
      return {
        key: 'two',
            ...
            children: [
              {
                ...
                attrs: {
                  type: 'primary',
                  size: ComponentSize,
                  loading: this.shopStoreLoadingFlag
                }
              }
            ]
          },
  ...
  methods: {
    ...
    submitHandler() {
      ...
      this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopStoreLoading', loading: true });
      let params = setQueryParams(this.formData, this.$store);
      let { start_date, end_date } = setQueryTimeAddTail(this.formData);
      params['start_date'] = start_date;
      params['end_date'] = end_date;
      alertQueryConditionWrong(params, this.$store, this.urlParams, ['shopStoreLoading', 'shopStoreParams',
        'shopStore']);
      let newParams = {
        url: 'getShopStoreList',
        tableData: 'tableShopStore',
        pageingFlag: ++this.pageNum
      };
      this.newPageingParams = newParams;
      this.$store.dispatch('info/getParams', { newParams, oneTableName: 'shopStoreNewParams', newParamsName: 'shopStore' });
      if (this.formData) {
  ...
  mounted() {
    ...
    sendOnceQuery(this.$store, 'shopStore', 'shopStoreParams', this.tableData, this.urlParams, 'shopStoreLoading');
    this.totalData = this.syncConfig.config;
    this.getShopIndex();
  },
  updated() {
    let l = this.totalData.length;
    this.totalData[l - 2].children[0].attrs['loading'] = this.shopStoreLoadingFlag;
  },
    ...
    next(vm => {
      if (vm.titleArray && vm.titleArray.length > 1) {
        ...
        if (vm.titleArrayNum === 0 && !vm.ifNewTitleArray) {
          vm.$store.commit('info/setTableData', { response: null, dataName: 'tableShopStore' });
        }
      }
      vm.titleArrayNum = 0;
    });
  ...
  beforeRouteLeave(to, from, next) {
    this.$store.commit('info/setQueryLoading', { loadingFlag: 'shopStoreLoading', loading: false });
    next(true);
  }
};
</script>
